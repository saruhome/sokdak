...
| CI 유지보수 | Quality Gate의 `actions/checkout`, `actions/setup-node`를 v5로 올려 Node.js 20 런타임 경고를 해소했다. | 원격 Quality Gate 재검증 필요 |
...
| 캐릭터 상업 이용 권리 | 원본의 `tCorn pie` 서명 관련 상업 이용·수정·배포 조건을 확인하지 못했다. | 권리자에게 서면 확인 또는 대체 에셋 확보 |
...
현 상태에서 TypeScript 검사, Jest 21개 스위트·87개 테스트, Expo Doctor 21/21, release 설정 검사, 프로덕션 웹 번들 생성이 통과했다. npm 감사 결과는 위 의존성 항목으로 별도 추적하며, 자동 다운그레이드는 적용하지 않는다.
