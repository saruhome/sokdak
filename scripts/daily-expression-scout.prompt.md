...
1. 웹 검색으로 실제 쓰이는 문장 5개를 수집한다. X(트위터)·인스타 캡션·커뮤니티 글·유튜브 댓글·블로그 후기 등
   ...
   드라마에 자주 나오는 사투리 표현도 대상에 포함한다(부산·경상, 전라, 제주 등) — 어느 작품·어느 장면에서
   쓰였는지 확인 가능한 것만. 사투리도 확실성 기준은 아래 규칙과 동일하다.
...
4. `expression_candidates` 에 insert:
   ...
   - review_note: 아래 해당 사항을 적는다 — 비속어·성적 표현 여부, 윗사람에게 쓰면 무례한지,
     사투리면 지역·통용 범위, 용례를 어디서 확인했는지(작품·게시물).
...
- 무조건 5개를 채운다. 중복·확신 부족으로 후보가 빠지면 다른 situation·다른 문장으로 계속 찾아
  **확실한 것으로만** 5개를 맞춘다.
