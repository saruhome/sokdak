...
- 앱의 기본 배경색은 `#F6F2EA`로 고정한다. 저장소의 `#edeae3`는 모두 `#F6F2EA`로 교체한다.
- Claude Code가 기본 배경색 또는 공통 배경색을 다른 색으로 변경하려는 경우 임의로 수정하지 않는다.
- 변경이 필요하면 먼저 사용자에게 변경 이유, 대상 파일, 기존 색상, 제안 색상, 영향 범위를 설명하고 반드시 재확인을 요청한다.
- 사용자의 명시적인 승인 전에는 배경색 변경을 구현·커밋·푸시하지 않는다.
- 승인을 받지 못한 경우 기존 `#F6F2EA`를 유지한다.
