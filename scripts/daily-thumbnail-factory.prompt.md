...
```sql
-- 인기순 — Canva 쿼터로 처리량이 적으니 많이 저장된 단어부터 (운영자 지시 2026-09-10)
...
**오늘 작업분 선별(운영자 지시 2026-09-10 — 감정 그룹 우선)**: 감정 그룹은 DB에 없고
1단계의 캐릭터 사용 판단으로만 정해지므로, 위 30건 풀을 먼저 분류한 뒤 최대 10개를 고른다:
1. 각 후보를 감정 표현형 / 서사형 / 비속어로 분류하고, 감정 표현형은 3단계 매핑 표의
   감정 그룹을 배정한다.
2. **같은 감정 그룹 2개 이상 묶이는 단어들부터 채운다**(그룹당 최대 4개 — 생성 1회로
   최대 4장이 나오는 배치 규칙을 최대로 쓰는 순서). 그룹 간 순서는 풀 안의 인기순을 따른다.
3. 남는 자리는 인기순대로 단독 감정형 → 비속어로 채운다. **서사형은 건너뛴다**
   (운영자 지시 2026-09-10: 서사형은 아침 검수의 장면 시트 트랙 — 캐릭터가 뜻을 연기하는
   ChatGPT 6칸 시트 — 이 담당한다. 공장은 감정형 합성과 비속어 배경만 처리).
분류 결과(그룹 구성)를 보고에 한 줄 남긴다.

## 1. 배경 생성 (Canva MCP) — v3 중앙 무대

generate-design, design_type: youtube_thumbnail.

**배치 규칙(운영자 승인 2026-09-07 — 쿼터 4배 활용)**: generate-design 1회는 후보 4장을
돌려주므로 버리지 말고 나눠 쓴다. 감정 표현형(캐릭터 합성) 단어는 **같은 감정 그룹**
(3단계 매핑 표 기준) 단어를 최대 4개 묶어 무대 1회만 생성하고, 검수를 통과한 후보를
단어별로 1장씩 배정한다(포즈·방향은 단어마다 다르게 순환). 서사형·비속어는 뜻을 소품으로
그려야 해서 단어당 1회를 유지하되, 탈락하지 않은 여분 후보가 뒤 단어의 무드와 맞으면
재생성 없이 재사용한다. 후보마다 create-design-from-candidate → export는 개별로 진행.
...
포즈 분위기 매핑(2026-09-05 호랭 36종, 2026-09-10 짹이 36종 유건 수정본 편입 —
파일명은 poses-clean 기준, *-t2m는 좌우 반전 변주):
...
- 일상·생활: horang-cook(요리), horang-clean-mop(청소), horang-blanket-peek(이불 빼꼼),
  horang-sleep-zzz(잠·귀찮음), jjaeki-cook, jjaeki-clean-broom, jjaeki-blanket-peek,
  jjaeki-sleep-zzz
- 긍정·인정: horang-thumbsup(엄지척·인정·알잘딱), horang-ok-sign(OK·ㅇㅋ),
  horang-muscle(힘자랑·갓생), horang-flower-shy(수줍·설렘), jjaeki-thumbsup, jjaeki-ok-sign,
  jjaeki-muscle, jjaeki-flower-shy, jjaeki-giggle-shy(입 가리고 수줍)
- 웃음: horang-laugh-tears(눈물 나게 폭소), horang-giggle(킥킥), jjaeki-laugh-loud
