...
- ✅ 앱 언어 확장 — 현재 **노출 7개**: 한/영/일/베트남/스페인/독일/터키(tr은 2026-09-02부터 노출).
  zh/ar은 준비만 되고 미노출(활성화 체크리스트는 해당 파일 헤더, ar은 RTL 선행 필수 —
  메모리 project_sokdak_hidden_locales 참고). `languageStore.ts` 전체 문자열 + 로컬
  `Record<Language>` 콘텐츠 블록 번역, `constants/categories.ts`에 언어별 name* 컬럼.
...
(검색에만 걸리고 화면에는 안 나온다 — `src/features/dictionary/model/wordSearch.ts`의 `alias` 필드).
표제어를 나누면 같은 단어가 검색 결과에 두 번 뜬다. 어느 쪽을 표제어로 삼느냐는 두 경우가 다르다.

| | 표기 변이 | 형태 변이 |
|---|---|---|
| 무엇 | 문자 체계가 다름 (알파벳 ↔ 한글) | 같은 한글 안에서 길이가 다름 (원말 ↔ 줄임말) |
| 예 | TMI / 티엠아이, 존맛탱 / JMT / 제이엠티 | 커터칼퇴근 / 커터칼퇴, 로판 / 로맨스판타지 |
| 표제어 | 국내에서 실제로 더 흔히 쓰는 표기 | 실제로 더 널리 쓰이는 형태 (원말일 수도 줄임말일 수도) |
| aliases | 나머지 표기 전부 | 나머지 형태 |

- 표기 변이에서 원말이 이미 표제어면(존맛탱) 새 표기(JMT·제이엠티)는 alias로 흡수한다.
- 형태 변이는 길이가 아니라 통용도로 정한다: 언론·자막이 '커터칼퇴근'을 쓰니 그쪽이 표제어이고,
  장르명은 '로판'이 압도적이라 줄임말이 표제어다. 판단이 갈리면 기사 검색으로 확인하고 근거를
  마이그레이션 주석에 남긴다.
- 어느 경우든 "어느 쪽으로 검색해도 항목 하나"라는 사실을 `usage`(5개 언어 `usage_i18n` 포함)에
  적고, 두 형태가 같은 말이라는 설명은 `definition`에 넣는다.
- 등록 전에 중복부터 확인한다 — `words.word`에 UNIQUE 제약이 없어 같은 표제어가 조용히 들어간다:
